...
<div class="bg-white rounded-lg shadow">
    ...
    <form method="POST" action="{{ route('admin.testimonials.update', $testimonial->id) }}" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea id="testimonial" 
                          ...
                          rows="6"
                          required
                          class="w-full px-4 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent @error('testimonial') border-red-500 @enderror">{{ old('testimonial', $testimonial->testimonial) }}</textarea>
                @error('testimonial')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
