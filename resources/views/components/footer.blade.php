<footer class="bg-neutral-900 text-neutral-300">
    <div class="container-custom section-padding">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            {{-- About --}}
            <div>
                <a href="{{ route('home') }}" class="inline-block mb-4">
                    <img
                        src="{{ asset('images/logo.png') }}"
                        alt="{{ config('app.name') }}"
                        class="h-12 w-auto brightness-0 invert"
                    >
                </a>
                <p class="text-sm mb-4 leading-relaxed">
                    {{ config('app.name') }} helps organizations build strong teams and unlock strong profits through strengths-based development.
                </p>
            </div>

            {{-- Quick Links --}}
            <div>
                <h3 class="text-white font-display font-bold text-lg mb-4">Quick Links</h3>
                <ul class="space-y-3 text-sm">
                    <li>
                        <a href="{{ route('home') }}" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Home
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('pages.show', 'sales-courses') }}" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Sales Courses
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('strengths-programme') }}" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Strengths Programme
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('blog.index') }}" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Blog
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('about-us') }}" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            About Us
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('contact') }}" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Contact
                        </a>
                    </li>
                </ul>
            </div>

            {{-- Services --}}
            <div>
                <h3 class="text-white font-display font-bold text-lg mb-4">Services</h3>
                <ul class="space-y-3 text-sm">
                    <li>
                        <a href="#" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Team Development
                        </a>
                    </li>
                    <li>
                        <a href="#" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Leadership Training
                        </a>
                    </li>
                    <li>
                        <a href="#" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Workshops
                        </a>
                    </li>
                    <li>
                        <a href="#" class="hover:text-primary-400 hover:translate-x-1 inline-block transition-all">
                            Keynote Talks
                        </a>
                    </li>
                </ul>
            </div>

            {{-- Contact --}}
            <div>
                <h3 class="text-white font-display font-bold text-lg mb-4">Contact</h3>
                <ul class="space-y-3 text-sm">
                    <li class="text-white font-semibold">
                        Eberhard Niklaus
                    </li>
                    <li>
                        <a href="tel:+27832948033" class="hover:text-primary-400 transition-colors">
                            +27 83 294 8033
                        </a>
                    </li>
                    <li>
                        <a href="mailto:welcome@eberhardniklaus.co.za" class="hover:text-primary-400 transition-colors">
                            welcome@eberhardniklaus.co.za
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('contact') }}" class="hover:text-primary-400 transition-colors">
                            Get in Touch
                        </a>
                    </li>
                </ul>

                {{-- Social Media --}}
                <div class="mt-6 flex space-x-3">
                    <a href="#" class="w-10 h-10 rounded-lg bg-neutral-800 flex items-center justify-center hover:bg-primary-600 transition-all hover:-translate-y-1">
                        <span class="sr-only">Facebook</span>
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"/>
                        </svg>
                    </a>
                    <a href="#" class="w-10 h-10 rounded-lg bg-neutral-800 flex items-center justify-center hover:bg-primary-600 transition-all hover:-translate-y-1">
                        <span class="sr-only">LinkedIn</span>
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                        </svg>
                    </a>
                    <a href="#" class="w-10 h-10 rounded-lg bg-neutral-800 flex items-center justify-center hover:bg-primary-600 transition-all hover:-translate-y-1">
                        <span class="sr-only">Twitter</span>
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84"/>
                        </svg>
                    </a>
                </div>
            </div>
        </div>

        {{-- Copyright --}}
        <div class="mt-12 pt-8 border-t border-neutral-800">
            <div class="flex flex-col md:flex-row justify-between items-center text-sm text-neutral-400">
                <p>&copy; {{ date('Y') }} {{ config('app.name') }}. All rights reserved.</p>
                <div class="flex space-x-6 mt-4 md:mt-0">
                    <a href="#" class="hover:text-primary-400 transition-colors">Privacy Policy</a>
                    <a href="#" class="hover:text-primary-400 transition-colors">Terms of Service</a>
                </div>
            </div>
        </div>
    </div>
</footer>
